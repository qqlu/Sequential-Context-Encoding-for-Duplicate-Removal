...
def train(box_feature_variable, box_score_variable, box_box_variable, box_label_variable, box_origin_score_variable, model, model_optimizer, criterion=None, pos_neg_weight=100):
    ...
    # print(np.where(box_label_np==1)[0].shape[0]) 
    # print(box_label_np.shape[0])
    # precision_all = 0.
    # recall_all = 0.
    # for cls_index in range(80):   
        # precision, recall, _ , _ = score(output_record_np, box_label_np, labels=[1])
    precision, recall, _ , _ = score(output_record_np, box_label_np, labels=[1])
        # precision_all += float(precision)
        # recall_all += float(recall)
    # precision = precision_all / 80.
    # recall = recall_all / 80. 
    # precision = float(precision)
    # print(precision.shape)
    # print('precision:{}, recall:{}'.format(precision, recall))
    # return loss.data[0], precision[1], recall[1]
...
def solver(model, data_loader, n_epochs, output_dir, print_every=1, save_every=1, learning_rate=0.01, step=10, pos_neg_weight=100, load_file=None, continue_epochs=None, continue_iters=None):
    ...
    for epoch_index in range(n_epochs):
        ...
        if first_count is None:
            ...
            count = first_count
        for box_feature, rank_score, box_box, box_label, box_weight, box_score in data_loader:
        # for box_feature, rank_score, box_box, box_label in data_loader:
            ...
            box_origin_score_variable = Variable(box_score).cuda()
            criterion = nn.BCELoss(weight=box_weight.cuda())
            # criterion = nn.BCELoss(weight=box_weight.cuda())
            # criterion = nn.BCELoss(weight=box_weight.cuda())
            # print box_score_fusion_variable.size()
            loss, pos_accuracy, neg_accuracy = train(box_feature_variable, box_score_variable, box_box_variable, box_label_variable, box_origin_score_variable,  
