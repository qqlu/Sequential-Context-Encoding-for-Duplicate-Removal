...
class Encoder_Decoder(nn.Module):
    ...
    def forward(self, all_class_box_feature_variable, all_class_box_box_variable, all_class_box_score_variable, all_class_box_origin_score_variable, unique_class, unique_class_len):
        ...
        for j in range(80):
            ...
            class_encoder_input_o = encoder_input[start:end, :, :]
            # print(class_encoder_input_o[0,:,:])
            class_encoder_input = flip(class_encoder_input_o, 0)
            # print(class_encoder_input[-1, :,:])
            # input()
            # inv_idx = torch.arange(class_encoder_input_o.size(0)-1, -1, -1).long().cuda()
            # class_encoder_input = class_encoder_input_o[inv_idx,:,:]

            # class_encoder_input = class_encoder_input[::-1, :, :]
