...
class Encoder_Decoder(nn.Module):
    ...
    def forward(self, boxes_feature, boxes_box_score, boxes_box, num_proposals):
        ...
        for j in range(self.n_layers):
            ...
            decoder_input = torch.add(encoder_input, decoder_output)
            decoder_output_final, hidden_decoder_final = self.encoder_rnn(decoder_input, hidden_decoder_final)

        output = self.sigmoid(self.out(decoder_output_final))
        # print(output)
        return output
