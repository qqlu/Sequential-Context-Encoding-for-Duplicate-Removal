...
def train(box_feature_variable, box_score_variable, box_box_variable, box_label_variable, box_num_variable, model, model_optimizer, box_weight_variable, pos_neg_weight=100):
    model_optimizer.zero_grad()

    output_record, output_label, output_weights = model(box_feature_variable, box_score_variable, box_box_variable, box_label_variable, box_weight_variable, box_num_variable)
...
def solver(model, data_loader, n_epochs, output_dir, print_every=1, save_every=1, learning_rate=0.01, step=10, pos_neg_weight=100, load_file=None, continue_epochs=None, continue_iters=None):
    ...
    for epoch_index in range(n_epochs):
        ...
        if first_count is None:
            ...
            count = first_count
        for box_feature, rank_score, box_box, box_label, box_weight, box_num in data_loader:
        # for box_feature, rank_score, box_box, box_label in data_loader:
            ...
            box_num_variable = Variable(box_num).cuda()
            box_weight_variable = Variable(box_weight).cuda()

            loss, pos_accuracy, neg_accuracy = train(box_feature_variable, box_score_variable, box_box_variable, box_label_variable, 
                        box_num_variable, model, model_optimizer, box_weight_variable)
            count += 1
