...
class MultiDataset(Dataset):
    def __init__(self, base_path, txt_name):
        ...
        self.single_class_search_table = self.single_class_rank_table()
    ...
    def single_class_rank_table(self):
        table = np.zeros((1000, 128), dtype=np.float)
        for i in range(1000):
            for j in range(64):
                table[i, 2*j:2*j+2] = i / (10000 ** (2*j/128.0))
        table[:, 0::2] = np.sin(table[:, 0::2])
        table[:, 1::2] = np.cos(table[:, 1::2])
        return table
...
class TrainDataset(MultiDataset):
    ...
    def __getitem__(self, idx):
        ...
        single_class_rank_score = np.zeros((num_box, 80*128))
        for ii in range(ranks.shape[0]):
            for jj in range(ranks.shape[1]): 
                rank_score[ii, jj*32: (jj+1)*32] = self.search_table[int(ranks[ii, jj]), :]
                single_class_rank_score[ii, jj*128: (jj+1)*128] = self.single_class_search_table[int(ranks[ii, jj]), :]
        ...
        all_class_box_score = np.zeros((num_class_box, 128))
        ...
        for ii in range(80):
            ...
            all_class_box_score[start:end, 0:128] = single_class_rank_score[class_sort_index, ii*128:(ii+1)*128]
