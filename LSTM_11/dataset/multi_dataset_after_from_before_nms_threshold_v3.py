...
class TrainDataset(MultiDataset):
    ...
    def __getitem__(self, idx):
        ...
        for ii in range(80):
            ...
            final_resort = save_score_final[final_valid, 0:1].argsort(axis=0)[::-1].reshape(-1)
            final_valid_resort = final_valid[final_resort] 
            final_sort = valid_sort[final_valid_resort]
            ...
            # input()
            all_class_box_origin_score[start:start+filter_num, :] = np.tile(save_score_final[start:end, 0:1][final_valid_resort, 0:1], 32)
            ...
            all_class_box_label[start:start+filter_num, 0:1], all_class_box_weight[start:start+filter_num, 0:1] = stage_full_assign_weight_slack_has_class_v6(all_class_box_origin_box[start:start+filter_num, :], all_class_box_origin_score[start:start+filter_num, 0:1], gts_box, ii, weight=self.weight)
        # final box
