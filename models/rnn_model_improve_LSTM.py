...
class Encoder_Decoder(nn.Module):
    ...
    def forward(self, boxes_feature, boxes_box_score, boxes_box, num_proposals):
        hidden_encoder, c_hidden_encoder = self.initHidden()
        ...
        for j in range(self.n_layers):
            ...
            encoder_output, (hidden_encoder, c_hidden_encoder) = self.encoder_rnn(encoder_input, (hidden_encoder, c_hidden_encoder))
            hidden_decoder = hidden_encoder
            c_hidden_decoder = c_hidden_encoder
            decoder_output, (hidden_decoder, c_hidden_decoder) = self.encoder_rnn(encoder_input, (hidden_decoder, c_hidden_decoder))
    ...
    def initHidden(self):
        ...
        c_result = Variable(torch.zeros(1, 1, self.hidden_size))
        if self.use_cuda:
            return result.cuda(), c_result.cuda()
        else:
