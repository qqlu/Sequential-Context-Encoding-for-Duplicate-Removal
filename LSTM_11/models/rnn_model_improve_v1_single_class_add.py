...
class Encoder_Decoder(nn.Module):
    def __init__(self, hidden_size, output_size=1, n_layers=1, use_cuda=True):
        ...
        self.decoder_rnn_final = nn.GRU(hidden_size, hidden_size)
        # shared weights
    ...
    def forward(self, boxes_feature, boxes_box_score, boxes_box, num_proposals):
        ...
        hidden_decoder_class = hidden_encoder
        decoder_output_class, hidden_decoder_class = self.decoder_rnn_class(encoder_input, hidden_decoder_class)
        hidden_decoder_final = hidden_encoder
        decoder_input = torch.add(encoder_input, decoder_output_class)
        decoder_output_final, hidden_decoder_final = self.decoder_rnn_class(decoder_input, hidden_decoder_final)
