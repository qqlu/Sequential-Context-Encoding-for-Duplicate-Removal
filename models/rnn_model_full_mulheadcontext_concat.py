...
class Encoder_Decoder(nn.Module):
    def __init__(self, hidden_size, output_size=1, n_layers=1, use_cuda=True, attn_type='dot', multi_head=4, context_type=None):
        ...
        # self.reduction = nn.Linear(hidden_size*2, hidden_size*2)
        
        # self.atten = GlobalAttention(hidden_size*2, attn_type=attn_type)
        # context
        self.context_type = context_type
        if self.context_type is not None:
            self.context_gate = nn.ModuleList([context_gate_factory(self.context_type, hidden_size, hidden_size*2, hidden_size*2, int(hidden_size*2/self.multi_head)) for i in range(self.multi_head)])
    ...
    def forward(self, all_class_box_feature_variable, all_class_box_box_variable, all_class_box_score_variable, all_class_box_origin_score_variable, unique_class, unique_class_len):
        ...
        for j in range(80):
            ...
            for qq in range(self.multi_head):
                ...
                if self.context_type is not None:
                    ...
                    class_final_mulhead_output_temp = class_final_mulhead_output_temp.view(-1, 1, int(self.hidden_size*2/self.multi_head))
        ...
        output = self.sigmoid(self.out(final_output.squeeze(1)))
