...
class Encoder_Decoder(nn.Module):
    def __init__(self, hidden_size, output_size=1, n_layers=1, use_cuda=True, attn_type='dot', context_type=None):
        ...
        # decoder_rnn
        self.decoder_rnn = nn.ModuleList([nn.GRU(hidden_size*2, hidden_size, bidirectional=True) for i in range(80)])
    ...
    def forward(self, all_class_box_feature_variable, all_class_box_box_variable, all_class_box_score_variable, all_class_box_origin_score_variable, unique_class, unique_class_len):
        ...
        for j in range(80):
            ...
            class_hidden_linear = self.hidden_linear(class_encoder_hidden.view(1,1,-1))
            class_hidden_linear = class_hidden_linear.expand_as(class_encoder_input)
            class_decoder_input = torch.cat((class_encoder_input, class_hidden_linear), 2)
   
            class_decoder_output, class_decoder_hidden = self.decoder_rnn[j](class_decoder_input, class_encoder_hidden) 
