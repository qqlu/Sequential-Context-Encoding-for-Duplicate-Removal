...
from utils.bbox_ops import jaccard
...
class TrainDataset(MultiDataset):
    ...
    def __getitem__(self, idx):
        ...
        label_num = 0
        for ii in range(80):
            equal_row_index = np.where(box_box[:, ii*4:ii*4+1]!=box_box[:,ii*4+2:ii*4+3])[0]
            equal_col_index = np.where(box_box[:, ii*4+1:ii*4+2]!=box_box[:,ii*4+3:])[0]
            equal_index = np.intersect1d(equal_row_index, equal_col_index)    
            valid_index = np.where(box_score[equal_index, ii]>=self.score_thresh)[0]
            cls_list.append(equal_index[valid_index])
            valid_num = len(equal_index[valid_index])
            cls_num += valid_num
            if not valid_num==0:
                unique_class[ii] = 1
            label_num += len(np.where(box_label[equal_index[valid_index],ii]==1)[0])
        ...
        if self.phase == 'train':
            return all_class_box_feature, all_class_box_box, all_class_box_score, all_class_box_label, all_class_box_weight, all_class_box_origin_score, all_class_box_origin_box, unique_class, unique_class_len, phase_np
...
def unique_collate(batch):
    ...
    if batch[0][-1][0]==1 or batch[0][-1][0]==2:
        ...
        # aa.extend(torch.from_numpy(np.array([batch[0][-2]])))
        # print(len(aa))
        return aa
    else:
        aa = [torch.FloatTensor(batch[0][i]) for i in range(len(batch[0])-2)]
...
if __name__ == '__main__':
    ...
    for i in range(num):
       all_class_box_feature, all_class_box_box, all_class_box_score, all_class_box_label, all_class_box_weight, all_class_box_origin_score, all_class_box_origin_box, unique_class, unique_class_len, phase_np = train[i]
       # print(all_class_box_feature.shape[0])
       # if i==20:
       #      break
       all_class_box_origin_box_variable = Variable(torch.FloatTensor(all_class_box_origin_box)).cuda()
       for j in range(80):
            if(unique_class[j]==0):
                continue
            start = int(unique_class_len[j])
            end = int(unique_class_len[j+1])
            class_box = all_class_box_origin_box_variable[start:end, :].clone()
            # class_iou = Variable(torch.ones(class_box.size(0), class_box.size(0)).cuda())- jaccard(class_box, class_box)
            class_iou = jaccard(class_box, class_box)
