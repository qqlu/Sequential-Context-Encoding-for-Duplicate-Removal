...
from utils import stage2_assign_weight_single_class
...
class TrainDataset(MultiDataset):
    ...
    def __getitem__(self, idx):
        ...
        proposals_feature_nms, proposals_score_nms, proposals_box_nms, proposals_label, proposals_class_label, proposals_weight, proposals_class_weight ,proposals_keep_np = stage2_assign_weight_single_class(box_feature, box_box, box_score, gts_box, weight=self.weight)
        ...
        box_class_label = proposals_class_label[sort_index, :]
        box_class_weight = proposals_class_weight[sort_index, :]
        ...
        # pos_len = len(np.where(box_class_label==1)[0])
        # all_len = len(box_class_label)
        # final_len = len(np.where(box_label==1)[0])
        # print('final_len:{}, pos_len:{}, all_len:{}, pos_ratio:{}, final_ratio:{}'.format(final_len, pos_len, all_len, pos_len/all_len, final_len/all_len))
        # input()
        if self.phase == 'train':
            return box_feature, rank_score, box_box, box_label, box_weight, box_class_label, box_class_weight 
...
def unique_collate(batch):
    if len(batch[0]) == 7:
        return torch.FloatTensor(batch[0][0]), torch.FloatTensor(batch[0][1]), torch.FloatTensor(batch[0][2]), torch.FloatTensor(batch[0][3]), torch.FloatTensor(batch[0][4]), torch.FloatTensor(batch[0][5]), torch.FloatTensor(batch[0][6])
    elif len(batch[0]) == 8:
