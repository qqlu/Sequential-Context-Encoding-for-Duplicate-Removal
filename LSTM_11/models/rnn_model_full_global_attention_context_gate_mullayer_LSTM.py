...
class Encoder_Decoder(nn.Module):
    def __init__(self, hidden_size, output_size=1, n_layers=1, use_cuda=True, attn_type='dot', context_type=None):
        ...
        self.encoder_rnn = nn.ModuleList([nn.LSTM(hidden_size, hidden_size, self.n_layers, bidirectional=self.bidirectional) for i in range(80)])
        
        # decoder_rnn
        self.decoder_rnn = nn.ModuleList([nn.LSTM(hidden_size, hidden_size, self.n_layers, bidirectional=self.bidirectional) for i in range(80)])
    ...
    def forward(self, all_class_box_feature_variable, all_class_box_box_variable, all_class_box_score_variable, all_class_box_origin_score_variable, unique_class, unique_class_len):
        ...
        hidden_encoder, hidden_encoder_c = self.initHidden()
        ...
        for j in range(80):
            ...
            class_encoder_output, (class_encoder_hidden, class_encoder_hidden_c) = self.encoder_rnn[j](class_encoder_input, (hidden_encoder, hidden_encoder_c)) 
            class_decoder_output, (class_decoder_hidden, class_decoder_hidden_c) = self.decoder_rnn[j](class_encoder_input, (class_encoder_hidden, class_encoder_hidden_c)) 
    ...
    def initHidden(self):
        if self.bidirectional:
            h_result = Variable(torch.zeros(self.n_layers * 2, 1, self.hidden_size))
            c_result = Variable(torch.zeros(self.n_layers * 2, 1, self.hidden_size))
        else:
            h_result = Variable(torch.zeros(self.n_layers * 1, 1, self.hidden_size))
            c_result = Variable(torch.zeros(self.n_layers * 1, 1, self.hidden_size))

        if self.use_cuda:
            return h_result.cuda(), c_result.cuda()
        else:
