...
def train(box_feature_variable, box_score_variable, box_box_variable, box_label_variable, model, model_optimizer, criterion=None, pos_neg_weight=100):
    ...
    # print(np.unique(box_label_np))
    # print('box_label_np:', box_label_np.reshape(-1))
    # print('box_label_np_length:', box_label_np.shape[0])
    # print(np.unique(output_record_np))
    # print('output_record_np:', output_record_np.reshape(-1))
    # print('output_record_np_length:', output_record_np.shape[0])
...
def solver(model, data_loader, n_epochs, output_dir, print_every=1, save_every=1, learning_rate=0.01, step=10, pos_neg_weight=100, load_file=None, continue_epochs=None, continue_iters=None):
    ...
    print_time_total = 0
    model_optimizer = optim.Adam(model.parameters(), lr=0.5, betas=(0.9, 0.999), eps=1e-08)
