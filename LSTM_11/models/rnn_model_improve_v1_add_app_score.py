...
class Encoder_Decoder(nn.Module):
    def __init__(self, hidden_size, output_size=1, n_layers=1, use_cuda=True):
        ...
        self.appear_score_linear = nn.Linear(hidden_size, hidden_size)

        self.all_feature_linear = nn.Linear(2*hidden_size, hidden_size)
    ...
    def forward(self, boxes_feature, boxes_box_score, boxes_box, num_proposals):
        ...
        encoder_box_feature_score = F.relu(self.appear_score_linear(torch.add(encoder_box_feature, encoder_box_score)))

        # encoder_all_feature = encoder_box_score

        encoder_all_feature = torch.cat((encoder_box_feature_score, encoder_box_box), 1)
        ...
        decoder_output, hidden_decoder = self.decoder_rnn(encoder_input, hidden_decoder)
