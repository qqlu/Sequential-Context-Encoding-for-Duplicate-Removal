...
class TrainDataset(MultiDataset):
    ...
    def __getitem__(self, idx):
        ...
        for ii in range(80):
            ...
            # final_valid = np.where(save_score_final[start:end, 0] >= 0)[0]
            final_valid = np.where(save_score_final[start:end, 0] <= 0.2)[0]
            # print(save_score_final.shape)
            # final_temp = save_score_origin[start:end, 0:1][final_valid, 0].argsort()[::-1]
            # final_temp = save_score_final[start:end, 0:1][final_valid, 0].argsort()[::-1]
            # final_sort = valid_sort[final_valid[final_temp]]
            ...
            all_class_box_weight[start:start+filter_num, 0] = box_weight[final_sort, ii]
            all_class_box_label[start:start+filter_num, 0] = box_label[final_sort, ii]

            all_class_box_box[start:start+filter_num, :] = np.tile(box_box[final_sort, ii*4:(ii+1)*4], 8)
            # print(np.concatenate((save_score_origin[start:end, 0:1][final_valid, 0:1], box_score[final_sort, ii].reshape(-1,1)), axis=1))
            # print(save_score_stage1[start:end, 0:1])
            # print()
            # input()
            all_class_box_origin_score[start:start+filter_num, :] = np.tile(save_score_final[start:end, 0:1][final_valid, 0:1], 32)
            # all_class_box_origin_score[start:start+filter_num, :] = np.tile(save_score_origin[start:end, 0:1][final_valid, 0:1], 32)
            # all_class_box_origin_score[start:start+filter_num, :] = np.tile(save_score_final[start:end, 0:1][final_temp, 0:1], 32)
            # all_class_box_origin_score[start:end, :] = np.tile(box_score_final[final_sort, ii:ii+1], 32)
        ...
        # print(save_score_stage1[0:1])
        # print(box_score[final_sort, ii])
        # input()
        # final box
        ...
        all_class_box_weight_final = np.ones((all_class_num, 1))
        all_class_box_label_final = np.zeros((all_class_num, 1))
        ...
        for ii in range(80):
            ...
            all_class_box_weight_final[start_new:end_new, 0] = all_class_box_weight[start_old:end_old, 0]
            all_class_box_label_final[start_new:end_new, 0] = all_class_box_label[start_old:end_old, 0]
        ...
        # print(unique_class_len)
        # print(unique_class_len_final)
        # input()
        # for ii in range(80):
        #     start_old = int(unique_class_len[ii])
        #     filter_num = int(unique_class_len_sta[ii])
        ...
        #     start_new = int(unique_class_len_final[ii])
        #     end_new = int(unique_class_len_final[ii+1])
        #     print(np.concatenate((all_class_box_origin_score[start_old:end_old, 0:1], all_class_box_origin_score_final[start_new:end_new, 0:1].reshape(-1,1)), axis=1))
            
        #     # print('old')
        #     # print(np.concatenate((all_class_box_origin_box[start_old:end_old, :], all_class_box_origin_score[start_old:end_old, :]), axis=1))
        #     # print('new')
        #     # print(np.concatenate((all_class_box_origin_box_final[start_new:end_new, :], all_class_box_origin_score_final[start_new:end_new, :]), axis=1))
        #     input()
