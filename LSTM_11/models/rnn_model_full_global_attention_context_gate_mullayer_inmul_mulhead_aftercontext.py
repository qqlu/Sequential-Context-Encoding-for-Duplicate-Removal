    ...
    def forward(self, all_class_box_feature_variable, all_class_box_box_variable, all_class_box_score_variable, all_class_box_origin_score_variable, unique_class, unique_class_len):
        ...
        for j in range(80):
            ...
            for qq in range(self.multi_head):
                ...
                if self.context_type is not None:
                    class_final_mulhead_output_temp = self.context_gate(class_encoder_input.view(-1, class_encoder_input.size(2)), class_decoder_output.view(-1, class_decoder_output.size(2)), class_final_mulhead_output_temp.view(-1, class_final_mulhead_output_temp.size(2)))
        ...
        output = self.sigmoid(all_class_box_origin_score_variable[:, 0:1] * self.out(final_output.squeeze(1)))
