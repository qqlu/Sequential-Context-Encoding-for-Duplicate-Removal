...
from utils import assign_label, assign_label_80_with_nms
...
class TrainDataset(MultiDataset):
    def __init__(self, base_path, txt_name, use_mode, cls_list, overlap_thresh=0.5, score_thresh=0.01, phase='train', weight=2.):
    ...
    def __getitem__(self, idx):
        ...
        box_sort_score = box_score[valid_index, 1:]
        box_feature = box_feature[valid_index, :]

        box_box = box_box[valid_index, 4:]
        ...
        box_label, box_weight = assign_label_80_with_nms(box_box_origin, box_score, weight=self.weight)
        ...
        if self.phase == 'train':
            ...
            return box_feature, rank_score, box_box, box_label, box_score_origin, box_box_origin, img_id
...
def unique_collate(batch):
    ...
    if len(batch[0]) == 3:
        ...
        return torch.FloatTensor(batch[0][0]), torch.FloatTensor(batch[0][1]), torch.FloatTensor(batch[0][2]), torch.FloatTensor(batch[0][3]), torch.FloatTensor(batch[0][4]), torch.FloatTensor(batch[0][5]), torch.IntTensor([batch[0][6]])
