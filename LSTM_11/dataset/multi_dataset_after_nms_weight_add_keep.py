...
class TrainDataset(MultiDataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.phase == 'train':
            ...
            return box_feature, rank_score, box_box, box_label, box_weight, box_score_origin
...
def unique_collate(batch):
    ...
    if len(batch[0]) == 3:
        ...
        return torch.FloatTensor(batch[0][0]), torch.FloatTensor(batch[0][1]), torch.FloatTensor(batch[0][2]), torch.FloatTensor(batch[0][3]), torch.FloatTensor(batch[0][4]), torch.FloatTensor(batch[0][5])
    elif len(batch[0]) == 8:
