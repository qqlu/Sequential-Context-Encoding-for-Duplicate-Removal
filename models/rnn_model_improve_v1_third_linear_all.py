...
class Encoder_Decoder(nn.Module):
    def __init__(self, hidden_size, output_size=1, n_layers=1, use_cuda=True):
        ...
        # decoder output
        self.out_1 = nn.Linear(3*hidden_size, hidden_size)
    ...
    def forward(self, boxes_feature, boxes_box_score, boxes_box, num_proposals):
        ...
        for j in range(self.n_layers):
            ...
            decoder_output_final = torch.cat((encoder_input, encoder_output, decoder_output), 2)
            # print(decoder_output_final.size())
            # input()
        output = self.sigmoid(self.out_2(F.relu(self.out_1(decoder_output_final))))
        # print(output)
        return output
