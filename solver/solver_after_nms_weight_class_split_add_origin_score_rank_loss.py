...
def train(box_feature_variable, box_score_variable, box_box_variable, box_label_variable, all_class_box_feature_variable, all_class_box_score_variable, all_class_box_box_variable, all_class_box_label_variable, all_class_box_origin_score_variable, all_class_box_rank_score_variable, all_class_box_rank_label_variable, model, model_optimizer, criterion, criterion_rank, unique_class, unique_class_len):
    model_optimizer.zero_grad()

    input_length = box_feature_variable.size()[0]
    output_record = model(box_feature_variable, box_score_variable, box_box_variable, all_class_box_feature_variable, all_class_box_score_variable, all_class_box_box_variable, all_class_box_origin_score_variable, unique_class, unique_class_len)

    loss_tradi = criterion(output_record.view(-1, 1), all_class_box_label_variable)
    
    loss_rank = criterion_rank(output_record.view(-1), all_class_box_rank_score_variable.view(-1), all_class_box_rank_label_variable.view(-1))
    loss = loss_rank + loss_tradi
...
def solver(model, data_loader, n_epochs, output_dir, print_every=1, save_every=1, learning_rate=0.01, step=10, pos_neg_weight=100, load_file=None, continue_epochs=None, continue_iters=None):
    ...
    for epoch_index in range(n_epochs):
        ...
        # for box_feature, rank_score, box_box, box_label in data_loader:
            ...
            all_class_box_rank_score = all_class_box_origin_score[:,0:1].clone()
            ...
            all_class_box_rank_label_variable = Variable(torch.FloatTensor(all_class_box_rank_label)).cuda()          
            all_class_box_rank_score_variable = Variable(all_class_box_rank_score).cuda()
            ...
            loss, pos_accuracy, neg_accuracy = train(box_feature_variable, box_score_variable, box_box_variable, box_label_variable, 
                        all_class_box_feature_variable, all_class_box_score_variable, all_class_box_box_variable, all_class_box_label_variable, all_class_box_origin_score_variable, all_class_box_rank_score_variable, all_class_box_rank_label_variable,
                        model, model_optimizer, criterion, criterion_rank, unique_class_cuda, unique_class_len_cuda)
