...
class Encoder_Decoder(nn.Module):
    def __init__(self, hidden_size, output_size=1, n_layers=1, use_cuda=True, attn_type='dot', multi_head=4, context_type=None):
        ...
        if self.context_type is not None:
            self.context_gate = nn.ModuleList([context_gate_factory(self.context_type, hidden_size, hidden_size*2, hidden_size*2, hidden_size*2) for i in range(self.multi_head)])
    ...
    def forward(self, all_class_box_feature_variable, all_class_box_box_variable, all_class_box_score_variable, all_class_box_origin_score_variable, unique_class, unique_class_len):
        ...
        for j in range(80):
            ...
            for qq in range(self.multi_head):
                ...
                if self.context_type is not None:
                    class_final_mulhead_output_temp = self.context_gate[qq](class_encoder_input.view(-1, class_encoder_input.size(2)), class_decoder_output.view(-1, class_decoder_output.size(2)), class_final_mulhead_output_temp.view(-1, class_final_mulhead_output_temp.size(2)))
                    class_final_mulhead_output_temp = class_final_mulhead_output_temp.view(-1, 1, self.hidden_size*2)
        ...
        # decoder
        final_output = torch.cat(final_output_list).squeeze(1)
        # print(final_output.size())
        # print(all_class_box_feature_variable.size())
        final_output = torch.add(all_class_box_feature_variable, final_output)
