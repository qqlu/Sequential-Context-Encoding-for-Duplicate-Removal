...
def train(all_class_box_feature_variable, all_class_box_box_variable, all_class_box_score_variable, all_class_box_label_variable, all_class_box_origin_score_variable, all_class_box_origin_box_variable, unique_class_cuda, unique_class_len_cuda, model, model_optimizer, criterion=None, pos_neg_weight=100):
    model_optimizer.zero_grad()

    # input_length = box_feature_variable.size()[0]
    output_record = model(all_class_box_feature_variable, all_class_box_box_variable, all_class_box_score_variable, all_class_box_origin_score_variable, all_class_box_origin_box_variable, unique_class_cuda, unique_class_len_cuda).view(-1, 1)
    # print('label')
    # print(all_class_box_label_variable)
    # print('output')
    # print(output_record)
    # loss = criterion(all_class_box_label_variable,output_record)
    # print(torch.nonzero(output_record != output_record))
    output_record = output_record.view(-1, 1) * all_class_box_origin_score_variable[:, 0:1]
    loss = criterion(output_record, all_class_box_label_variable)
...
def solver(model, data_loader, n_epochs, output_dir, print_every=1, save_every=1, learning_rate=0.01, step=10, pos_neg_weight=100, load_file=None, continue_epochs=None, continue_iters=None):
    ...
    for epoch_index in range(n_epochs):
        ...
        if first_count is None:
            ...
            count = first_count
        for all_class_box_feature, all_class_box_box, all_class_box_score, all_class_box_label, all_class_box_weight, all_class_box_origin_score, all_class_box_origin_box, unique_class, unique_class_len in data_loader:
        # for box_feature, rank_score, box_box, box_label in data_loader:
            start = time.time()
            # img_name = int(img_id.numpy()[0])
            # print(img_id)
            # print('begin')
            ...
            loss, pos_accuracy, neg_accuracy = train(all_class_box_feature_variable, all_class_box_box_variable, all_class_box_score_variable, all_class_box_label_variable, all_class_box_origin_score_variable, all_class_box_origin_box_variable,
