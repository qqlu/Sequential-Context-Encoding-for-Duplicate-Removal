...
class Encoder_Decoder(nn.Module):
    def __init__(self, hidden_size, output_size=1, n_layers=1, use_cuda=True, attn_type='dot', context_type=None):
        ...
        # class hidden
        self.hidden_linear_1 = nn.Linear(hidden_size*80, 1024)
        self.hidden_linear_2 = nn.Linear(1024, 128)
        # decoder_rnn
    ...
    def forward(self, all_class_box_feature_variable, all_class_box_box_variable, all_class_box_score_variable, all_class_box_origin_score_variable, unique_class, unique_class_len):
        ...
        for j in range(80):
            if(unique_class[j]==0):
                hidden_final_encoder.append(self.initHidden())
                continue
            ...
            hidden_final_encoder.append(class_encoder_hidden)
        
        # class hidden
        hidden_class = torch.cat(hidden_final_encoder, 2)
        hidden_class = F.relu(self.hidden_linear_2(F.relu(self.hidden_linear_1(hidden_class))))
        ...
        for j in range(80):
            ...
            class_encoder_input = encoder_input[start:end, :, :]
            hidden_class_1 = torch.add(hidden_class, hidden_final_encoder[j])    
            class_decoder_output, class_decoder_hidden = self.encoder_rnn[j](class_encoder_input, hidden_class_1) 
        ...
        # input()
        output = self.sigmoid(self.out(final_output.view(-1, self.hidden_size*2)))
        # print(output)
        return output
