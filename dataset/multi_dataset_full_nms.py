...
class TrainDataset(MultiDataset):
    ...
    def __getitem__(self, idx):
        ...
        for ii in range(80):
            ...
            temp = box_score[valid_index, ii].argsort()[::-1]
            valid_sort = valid_index[temp]
            x1 = all_class_box_origin_box[valid_sort, 0:1]
            y1 = all_class_box_origin_box[valid_sort, 1:2]
            x2 = all_class_box_origin_box[valid_sort, 2:3]
            y2 = all_class_box_origin_box[valid_sort, 3:4]
            ...
            w = x2-x1+1
            h = y2-y1+1
            w_m = np.tile(w, x1.shape[0])
            h_m = np.tile(h, y1.shape[0])
            w_n = np.tile(w.reshape(1, -1), (x1.shape[0], 1))
            h_n = np.tile(h.reshape(1, -1), (y1.shape[0], 1))    
            # first
