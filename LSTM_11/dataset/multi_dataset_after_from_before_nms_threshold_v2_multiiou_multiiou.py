...
from utils.bbox_ops import stage_full_assign_weight_slack_has_class_v6_multiiou_multiiou
...
class TrainDataset(MultiDataset):
    ...
    def __getitem__(self, idx):
        ...
        all_class_box_weight = np.ones((cls_num, 10))
        all_class_box_label = np.zeros((cls_num, 10))
        ...
        for ii in range(80):
            ...
            all_class_box_label[start:start+filter_num, :], all_class_box_weight[start:start+filter_num, :] = stage_full_assign_weight_slack_has_class_v6_multiiou_multiiou(all_class_box_origin_box[start:start+filter_num, :], all_class_box_origin_score[start:start+filter_num, 0:1], gts_box, ii, weight=self.weight)
        # final box
        ...
        all_class_box_weight_final = np.ones((all_class_num, 10))
        all_class_box_label_final = np.zeros((all_class_num, 10))
