...
from models.rnn_model_full_global_attention_context_gate_multiiou_multiiou import Encoder_Decoder
from dataset.multi_dataset_after_from_before_nms_threshold_v2_multiiou_multiiou import TrainDataset, unique_collate
from solver.solver_full_nms_weight_multiiou_multiiou import solver, load_checkpoint
import torch
import argparse
import os
...
def parse_args():
    ...
    parser.add_argument('--final_score_thresh',
                        default=0.01,
                        type=float,
                        help='final_score_thresh')
...
if __name__ == '__main__':
    ...
    # dataset
    train = TrainDataset(args.base_path, args.img_list, args.use_mode, cls_list, weight=args.weight, final_score_thresh=args.final_score_thresh)
    train_loader = torch.utils.data.DataLoader(train, batch_size=1, num_workers=1, collate_fn=unique_collate, pin_memory=True, shuffle=True)
