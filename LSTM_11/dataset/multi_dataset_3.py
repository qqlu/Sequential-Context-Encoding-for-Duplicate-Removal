...
class TrainDataset(MultiDataset):
    ...
    def __getitem__(self, idx):
        ...
        # box_label, box_weight = assign_label(box_box_origin, gts_box, weight=100)
        box_label = assign_label_with_nms(box_box_origin, box_score)
...
def unique_collate(batch):
    ...
    if len(batch[0]) == 3:
        ...
        input()
        print('numpy:', batch[0][0][0,-1])
        box_feature_tensor = torch.from_numpy(batch[0][0].astype(np.float))
        rank_score_tensor = torch.from_numpy(batch[0][1].astype(np.float))
        box_box_tensor = torch.from_numpy(batch[0][2].astype(np.float))
        box_label_tensor = torch.from_numpy(batch[0][3].astype(np.float))
        return box_feature_tensor, rank_score_tensor, box_box_tensor, box_label_tensor
    elif len(batch[0]) == 5:
