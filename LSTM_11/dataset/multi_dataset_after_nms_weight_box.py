...
class MultiDataset(Dataset):
    ...
    def pos_list(self):
        list1 = np.zeros((1, 4))
        list1[0, 0] = 10000 ** (2*0/4.0)
        list1[0, 1] = 10000 ** (2*0/4.0)
        list1[0, 2] = 10000 ** (2*1/4.0)
        list1[0, 3] = 10000 ** (2*1/4.0)
        list1 = np.tile(list1, 320)
        return list1
...
class TrainDataset(MultiDataset):
    ...
    def __getitem__(self, idx):
        ...
        box_box_pos = box_box.copy()
        # calculate box_box_pos
        box_box_pos[:, 0::2] = np.log(box_box_pos[:, 0::2] / float(width) + 0.5)
        box_box_pos[:, 1::2] = np.log(box_box_pos[:, 1::2] / float(height) + 0.5)
        box_box_pos = box_box_pos.repeat(4).reshape(-1, 320*4)
        box_box_pos_table = np.tile(self.pos_list, box_box_pos.shape[0]).reshape(box_box_pos.shape[0], -1)
        box_box_final = box_box_pos * box_box_pos_table
        box_box_final[:, 0::2] = np.sin(box_box_final[:, 0::2])
        box_box_final[:, 1::2] = np.cos(box_box_final[:, 1::2])
        ...
        if self.phase == 'train':
            ...
            return box_feature, rank_score, box_box_final, box_label, box_weight
        elif self.phase == 'test':
            return box_feature, rank_score, box_box_final, box_label, box_score_origin, box_box_origin, img_id, box_keep_np
