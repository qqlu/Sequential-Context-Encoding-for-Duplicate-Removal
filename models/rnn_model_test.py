...
class Encoder_Decoder(nn.Module):
    def __init__(self, hidden_size, output_size=1, n_layers=1, use_cuda=True):
        ...
        self.score_linear = nn.Linear(80*32, hidden_size)
        self.box_linear = nn.Linear(320, hidden_size)
    ...
    def forward(self, boxes_feature, boxes_box_score, boxes_box, num_proposals):
        ...
        encoder_box_score = self.score_linear(boxes_box_score)
        ...
        encoder_input = torch.unsqueeze(encoder_all_feature, 1)
        # print(output.size())
    ...
    def initHidden(self):
        result = Variable(torch.zeros(1, 1, 3 * self.hidden_size))
        if self.use_cuda:
            return result.cuda()
        else:
            return result
