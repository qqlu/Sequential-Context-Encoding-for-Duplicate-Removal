...
import numpy as np
import random
from models.GlobalAttentionMask import GlobalAttention
...
class Encoder_Decoder(nn.Module):
    ...
    def forward(self, all_class_box_feature_variable, all_class_box_box_variable, all_class_box_score_variable, all_class_box_origin_score_variable, all_class_box_origin_box_variable, unique_class, unique_class_len):
        ...
        final_output_list = []
        # for j in range(80):
            # print(int(unique_class_len[j+1])-int(unique_class_len[j]))
        for j in range(80):
            if(unique_class[j]==0):
                continue
            ...
            class_iou = 1 - jaccard(class_box, class_box)
            ...
            class_final_output, _ = self.atten(class_decoder_output.permute(1, 0, 2), class_encoder_output.permute(1, 0, 2), class_iou)
            if self.context_type is not None:
